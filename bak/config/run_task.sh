...
if [ ! -f "$execute_file_path" ]; then
  echo "download:$execute_file_path ${APPLICATION_MANAGER_ADDRESS}/job/resource/${APPLICATION_ID}/${FILE_NAME}"
  curl -o $execute_file_path ${APPLICATION_MANAGER_ADDRESS}/job/resource/${APPLICATION_ID}/${FILE_NAME}
